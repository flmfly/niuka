...
public class Repository {
	...
	// lettuceConnectionFactory(@Value("${spring.redis.host}") String host,
	// @Value("${spring.redis.port}") int port, @Value("${spring.redis.timeout}")
	// long timeout) {
	...
	// SessionInterceptor(template));
	// }

	@Bean
	public MappedInterceptor sessionInterceptor() {
		return new MappedInterceptor(new String[] { "/**" }, new SessionInterceptor());
	}

	public static void main(String[] args) {
		SpringApplication.run(Repository.class, args);
	}
}
